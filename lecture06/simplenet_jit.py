...
def main(
    ...
    model = SimpLeNet.init(key=key_model_init)
    print(jax.tree.map(lambda leaf: leaf.shape, model))
    
    num_params = sum([leaf.size for leaf in jax.tree.leaves(model)])
    print(f"{num_params=}")


    print("initialise the optimiser and train step")
    opt_state = Adam.init(
            model=model,
            alpha=learning_rate,
            beta1=adam_beta1,
    ...
    def train_step(
        ...
        loss, grads = jax.value_and_grad(batch_cross_entropy)(model, x_batch, y_batch)
        
        # update optimiser, get model update, apply update rule
    ...
    for step in range(num_steps):
        ...
        # do the jittable computation of the train step
        ...
        digit_plot = vis_digits(
                digits=x_test[jnp.array([1,6,7,8])],
                labels=y_test[jnp.array([1,6,7,8])],
                model=model
            )
...
class Subsample2x2:
    ...
    @staticmethod
    def init(num_channels) -> Self:
        return Subsample2x2(
                weights=jnp.ones(shape=(num_channels, 1, 1)) / 4, 
                biases=jnp.zeros(shape=(num_channels, 1, 1))
                )
...
class SimpLeNet:
    ...
    def forward(
        ...
        x = einops.rearrange(image, "h w -> 1 h w") # -> 1x28x28
        x = scaled_tanh(self.C1.forward(x)) # -> 6x28x28
        x = scaled_tanh(self.S2.forward(x)) # -> 6x14x14
        x = scaled_tanh(self.C3.forward(x)) # -> 16x10x10
        x = scaled_tanh(self.S4.forward(x)) # -> 16x5x5
        x = scaled_tanh(self.C5.forward(x)) # -> 120x1x1
        x = jnp.ravel(x) # -> 120
    ...
    def init(
        ...
        return SimpLeNet(
            C1=SimpleConv2D.init(key=k1, kernel_size=5, num_channels_in=1, num_channels_out=6, padding=2),
            S2=Subsample2x2.init(num_channels=6),
            C3=SimpleConv2D.init(key=k2, kernel_size=5, num_channels_in=6, num_channels_out=16, padding=0),
            S4=Subsample2x2.init(num_channels=16),
            C5=SimpleConv2D.init(key=k3, kernel_size=5, num_channels_in=16, num_channels_out=128, padding=0),
            F6=AffineTransform.init(key=k4, num_inputs=128, num_outputs=84),
            Out=AffineTransform.init(key=k5, num_inputs=84, num_outputs=10)
            )
...
def batch_cross_entropy(
    ...
    vmapped_xent = jax.vmap(
            cross_entropy,
            in_axes=(None, 0, 0)
    )

    all_xents = vmapped_xent(
            model,
            xs,
            ys
        )

    return jnp.mean(all_xents)
